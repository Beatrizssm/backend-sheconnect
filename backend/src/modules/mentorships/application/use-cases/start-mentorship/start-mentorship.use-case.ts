...
@Injectable()
export class StartMentorshipUseCase {
  constructor(private readonly changeStatus: ChangeMentorshipStatusUseCase) {}

  execute(input: StartMentorshipInput): Promise<MentorshipEntity> {
    return this.changeStatus.execute({
      ...input,
      status: MentorshipStatus.EM_ANDAMENTO,
    });
  }
}
